<script lang="ts">
    export let top3 = false
</script>
<a href="/news/winners">Test</a>
<div on:click="{() => location.href='/news/final_round/'}" class="news-item dropshadow">
    <img class="news-img-small" src="/images/charles_torres_small.jpg" alt="charles_torres_small.jpg">
    <div>
        <span style="color: rgba(100, 100, 100, 0.5);"><i>September 18<sup>th</sup>, 2020 - 07:00 pm</i></span><br>
        <div style="line-height: 1.55;">
            <b>Results of the final round</b><br>
            We are excited to announce the winners of the 2<sup>nd</sup> international music composition competition "Future Symphony"!
            <span class="link">Read more</span>
        </div>
    </div>
</div>
<div on:click="{() => location.href='/news/1st_round/'}" class="news-item dropshadow">
    <img class="news-img-small" src="/images/news_1st_round_small.jpg" alt="news_1st_round_small.jpg">
    <div>
        <span style="color: rgba(100, 100, 100, 0.5);"><i>July 20<sup>th</sup>, 2020 - 10:00 pm</i></span><br>
        <div style="line-height: 1.55;">
            <b>Results of the 1<sup>st</sup> Round</b><br>
            We are excited to announce 7 finalists of the 2<sup>nd</sup> Future Symphony Competition! Participants listed below proceed to the 2<sup>nd</sup> round of the competition:
            <span class="link">Read more</span>
        </div>
    </div>
</div>
<div on:click="{() => location.href='/news/karoso/'}" class="news-item dropshadow">
    <img class="news-img-small" src="/images/news_karoso.png" alt="news_karoso.png">
    <div>
        <span style="color: rgba(100, 100, 100, 0.5);"><i>July 11<sup>th</sup>, 2020 - 4:00 pm</i></span><br>
        <div style="line-height: 1.55;">
            <b>Special prize for the best Lithuanian composition</b><br>
            The best Lithuanian composer this year will be awarded with a special prize founded by the competition’s main sponsor Saulius Karosas Foundation!
            <span class="link">Read more</span>
        </div>
    </div>
</div>
{#if top3}
    <div class="center"><a href="/news" class="link">Older news</a></div>
{/if}

<style lang="stylus">
    @require '../styles/news.styl'

    .center
        width 100%
        text-align center
</style>

